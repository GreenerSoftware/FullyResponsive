...
export async function view(response: ScloudResponse, template: string, context?: any, statusCode = 200): Promise<ScloudResponse> {
  const body = await njkView(template, context);
  return {
    ...response,
    statusCode,
    headers: {
...
const scloudGetHandler = async (request: ScloudRequest, handlerParameters: HandlerParameters): Promise<ScloudResponse> => {
  ...
  if (query.action === 'back') {
    ...
    const lastVisitIndex = previousPages.lastIndexOf(`${parameters.path}${queryParameterString}`);
    const newPreviousPages = [...previousPages.slice(0, lastVisitIndex), ...previousPages.slice(lastVisitIndex + 1)];
    await set('previousPages', newPreviousPages, response);
    // We'll be rendering the 'already previous' page, so we need to find
    // out what the 'previous previous' or 'new previous' page is for the
    // back link.
    newPreviousPage = newPreviousPages.at(-1);
...
class Page implements ServerRoute, CustomHandlers {
  ...
  constructor(parameters: PageParameters) {
    ...
    this.scloudHandler = async (request: ScloudRequest): Promise<ScloudResponse> => {
      ...
      await slackLog('scloudHandler', request.method, request.path, JSON.stringify(result));
      return result;
    };
